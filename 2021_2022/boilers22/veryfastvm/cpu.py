...
class Instruction:
    ...
    def __init__(self, tstr):
        def X06(tt):
            print("inside x06 tt-> " + str(tt) + "\nassert b>=0\nassert v<" + str(pow(2,32)))
            if tt.startswith("0x"):
                try:
                    v = int(tt, 16)
                except ValueError:
            ...
            return v
        def X17_X08(tt):
            print("inside x17_x08 tt-> " + str(tt) + "\nassert b>-1000\nassertv<1000")
            try:
                v = int(tt)
            except ValueError:
                assert False
            ...
            return v
        def X07(tt):
            print("inside x07 tt-> " + str(tt) + "\nassert tt[0] -> " + str(tt[0]) + " shud be 'r'\nassert v>=0 v<X14 ;" + str(X14) + ";")
            assert len(tt) == 2
            assert tt[0] == "r"
            try:
                v = int(tt[1])
...
class Cpu:
    ...
    def execute(self, ins):
        self.X02 += 1
        print(ins)
        print("INS OP : " + str(ins.op))
        if ins.op == "movc":
            self.X01[ins.X11] = ins.imm
            self.X05 += 1
        elif ins.op == "magic":
            print('magic')
            print(str(self.X00) + ' shud be 2')
            print(str(self.X01[0:4]) + ' shud be' + str(self.X04))
            if self.X00 == 2:
